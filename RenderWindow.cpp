...
HRESULT RenderWindow::InitialiseWindow(std::string& m_windowName, HINSTANCE _hInstance, int _nCmdShow)
{
    ...
    Renderer::GetInstance().GetHWND() = CreateWindow(m_windowName.c_str(), NULL, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
        rc.right - rc.left, rc.bottom - rc.top, NULL, NULL, _hInstance, NULL);

    if (!Renderer::GetInstance().GetHWND()) { return E_FAIL; }

    ShowWindow(Renderer::GetInstance().GetHWND(), _nCmdShow);
